...
-- 1.
select name,rollno from Student where team='R'
UNION
select name,rollno from Student where gender='F';

-- 2.
select team,count(team) from Student group by team;

-- 3.
select team, avg(math) from Student where gender = 'M' group by team order by avg(math);

-- 4.
select count(*) from Student group by gender,team order by team;
-- select * from Student group by team,gender order by team desc;
-- 5.
